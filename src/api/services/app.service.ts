...
export class AppService {
  constructor(
    @InjectModel(LiftLog.name) private liftLogModel: Model<LiftLogDocument>,
  ) {}

  async createLiftLog(
    createLiftLog: CreateLiftLogDto,
  ): Promise<LiftLogDocument> {
    return await new this.liftLogModel({
      title: createLiftLog.title,
      content: createLiftLog.content,
    }).save()
  }
